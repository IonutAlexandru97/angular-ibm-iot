...
@Directive({
  selector: '[clientSidebar],client-sidebar',
  host: {
    class: 'client-sidebar'
  },
  exportAs: 'clientSidebar'
})
export class SidebarDirective implements AfterViewInit, OnDestroy {
  ...
  enableScrollblock() {
    this.document.body.classList.add('client-scrollblock');
  }

  disableScrollblock() {
    this.document.body.classList.remove('client-scrollblock');
  }
